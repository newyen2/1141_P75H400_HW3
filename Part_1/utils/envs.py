...
# Construct a state that only consists of position and angle
class PartiallyObservableCartPole(gym.Env):
    ...
    def step(self, action):
        ...
        return np.array(self.state, dtype=np.float32)[[0,2]], reward, done, {}
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # 這裡維持舊版介面，只回傳 obs（因為 play_episode 也是這樣寫的）
        obs = np.array(self.state, dtype=np.float32)[[0, 2]]
        return obs
