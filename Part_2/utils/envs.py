...
class TimeLimit(gym.Wrapper):
    ...
    def step(self, action):
        ...
        # gymnasium 介面：step 回傳 5 個值
        observation, reward, terminated, truncated, info = self.env.step(action)
        self._elapsed_steps += 1

        if self._elapsed_steps >= self._max_episode_steps:
            # 若還沒結束 就用 TimeLimit 把它標成 truncated
...
def play_episode(env, policy, render=False):
    ...
    # gymnasium reset 可能回傳 obs 或 (obs, info)
    out = env.reset()
...
# Noisy CartPole-v0 Environment (converted to gymnasium API)
...
class NoisyCartPole(gym.Env):
    ...
    def __init__(self, render_mode="human"):
        ...
        # gymnasium 會在 reset(seed=...) 時幫你設定 self.np_random
        self.np_random = None
    ...
    def step(self, action):
        ...
        noise = self.np_random.uniform(low=-0.05, high=0.05)
        force *= (1 + noise)
        ...
        truncated = False  # 是否用 TimeLimit 來處理 episode 長度
    ...
    def render(self, mode="human"):
        # 保留原本的 pyglet render；如果你沒用 render 可以忽略這段
        screen_width = 600
        screen_height = 400
        ...
        if self.viewer is None:
            from gymnasium.envs.classic_control import rendering as pyglet_rendering
